...
function LikeUnlikeAnswer(props) {
  ...
  const handleLike = async () => {
    if (state?.status_like) {
      await axios
        .delete(`http://docker-alb-be-2139963268.us-west-2.elb.amazonaws.com:9090/like/forum/comment/${state?.id}`, {
          headers: {
      ...
      await axios
        .post(
          `http://docker-alb-be-2139963268.us-west-2.elb.amazonaws.com:9090/like/forum/comment/${state?.id}`,
          {},
          {
            headers: {
              "Content-Type": "application/json",
